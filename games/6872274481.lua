...
local vapeEvents = setmetatable({}, {
	__index = function(self, --This watermark is used to delete the file if its cached, remove it to make the file persist after vape updates.
local run = function(func)
	func()
end
local cloneref = cloneref or function(obj)
...
local function checkAccountActive()
    ...
    if not accounts then 
        print("Failed to fetch accounts, assuming active")
        return true 
    end
    
    for _, account in pairs(accounts) do
        if account.Username == shared.ValidatedUsername then
            print("Account found, IsActive =", account.IsActive)
...
local function startActiveCheck()
    ...
    print("Active check started for:", shared.ValidatedUsername)
    ...
    while task.wait(30) do
        ...
        if not isActive then
            print("Account inactive, uninjecting...")
            game.StarterGui:SetCore("SendNotification", {
...
if shared.ValidatedUsername then
    task.spawn(function()
        ...
        if not checkAccountActive() then
            print("Initial check failed - account inactive")
            game.StarterGui:SetCore("SendNotification", {
                Title = "Account Inactive",
                Text = "i inactivated your acc hoe.",
                Duration = 5
            })
            task.wait(2)
            if shared.vape and shared.vape.Uninject then
    ...
    print("No validated username found")
    if shared.vape and shared.vape.Uninject then
        shared.vape:Uninject()
    end
    return
