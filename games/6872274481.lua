            ...
            if callback then
                ...
                repeat
                    ...
                    if sword and (canAttack or (shouldSwing and lastTargetTime > 0)) then
                        ...
                        if #swingPlrs > 0 or #attackPlrs > 0 then
                            lastTargetTime = tick()
                            continueSwingCount = 0
                        ...
                        if #swingPlrs > 0 then
                            for _, v in swingPlrs do
                                ...
                                local angle = math.acos(localfacing:Dot((delta * Vector3.new(1, 0, 1)).Unit))
                                local swingAngle = math.rad(SwingAngleSlider.Value)
                        ...
                        if #attackPlrs > 0 then
                            for _, v in attackPlrs do
                                local delta = (v.RootPart.Position - selfpos)
                                local angle = math.acos(localfacing:Dot((delta * Vector3.new(1, 0, 1)).Unit))
                                local attackAngle = math.rad(AttackAngleSlider.Value)
                                if angle > (attackAngle / 2) then continue end

                                local alreadyAdded = false
                                for _, existing in attacked do
                                    if existing.Entity == v then
                                        alreadyAdded = true
                                        break
                                    end
                                end
